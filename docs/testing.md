...
| `tests/test_services.py` | `api/services.py` 全関数 | 85 | submit_job（time_limit・リソース超過バリデーション・cpu_millicores / memory_mib 設定含む）/ list_jobs（time_limit_ge・time_limit_lt フィルター含む）/ get_job / cancel（HELD 含む）/ hold_single・hold_bulk / release_single・release_bulk / delete（HELD skip 含む）/ reset（HELD ブロック含む）/ get_usage |
...
**合計: Python 231 + Rust (cli) 52 + Rust (cjobctl) 28 = 311 テスト**
