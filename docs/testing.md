...
| `tests/test_services.py` | `api/services.py` 全関数 | 83 | submit_job（time_limit・リソース超過バリデーション含む）/ list_jobs（time_limit_ge・time_limit_lt フィルター含む）/ get_job / cancel（HELD 含む）/ hold_single・hold_bulk / release_single・release_bulk / delete（HELD skip 含む）/ reset（HELD ブロック含む）/ get_usage |
...
| `src/main.rs` | `parse_duration` / `parse_time_limit_range` | 16 | 時間指定のパース（秒数 / s / m / h / d サフィックス / 空白 / 不正値 / オーバーフロー）/ time_limit 範囲指定のパース（両端指定 / 片端省略 / コロンなし / 空範囲 / 不正値） |
...
**合計: Python 229 + Rust (cli) 45 + Rust (cjobctl) 28 = 302 テスト**
