...
| `tests/test_determine_status.py` | `watcher/reconciler.py::determine_status` | 13 | K8s Job の conditions から DB ステータスへのマッピング。SUCCEEDED / FAILED / RUNNING / DeadlineExceeded / 条件なし等 |
| `tests/test_build_k8s_job.py` | `dispatcher/k8s_job.py` | 45 | K8s Job マニフェスト生成。ラベル / activeDeadlineSeconds 非設定（通常・sweep）/ リソース / 環境変数 / ボリューム / コマンドラッピング / toleration（デフォルト・カスタム・空）/ CPU limit バッファ（乗数適用・メモリ非適用・GPU 非適用・ミリコア入力）/ taint パース（正常系・異常系）/ `_extract_k8s_error_message` の body パース（message 取得・reason フォールバック・不正 JSON・message 無し dict）/ `create_k8s_job` の ApiException 分類（403+exceeded quota → QuotaExceededError・403 RBAC → PermanentK8sError で message 含む・429/503 → TemporaryK8sError・403 body 無し → PermanentK8sError・422 → PermanentK8sError） |
| `tests/test_services.py` | `api/services.py` 全関数 | 152 | submit_job（time_limit・リソース超過バリデーション・nominalQuota 考慮バリデーション・cpu_millicores / memory_mib 設定含む・RUNNING ジョブの MAX_QUEUED_JOBS_PER_NAMESPACE カウント除外）/ list_jobs（flavor・time_limit_ge・time_limit_lt フィルター含む）/ get_job / cancel（HELD 含む）/ hold_single・hold_bulk / release_single・release_bulk / set_single・set_bulk（QUEUED/HELD の各パラメータ変更・RUNNING/SUCCEEDED/CANCELLED スキップ・not_found・不正 flavor 拒否・GPU 非対応 flavor 拒否・リソース超過拒否・time_limit 超過拒否・複数フィールド同時変更・未指定フィールド保持・cpu_millicores/memory_mib 更新・マージ後バリデーション・一括操作）/ delete（HELD skip 含む）/ reset（HELD ブロック含む）/ get_usage（ResourceQuota あり・なし・namespace 分離含む）/ submit_sweep（クラスタ合計・nominalQuota 考慮バリデーション含む）/ list_flavors（quota 有無・nodes と quota の同時取得・flavor 既定イメージの返却）/ image 解決（`--image` 優先・flavor 既定 > fallback・flavor 既定なし時の fallback・`DEFAULT_FLAVOR` の既定適用・全て未解決時の 400・空文字の未指定扱い・旧 CLI 互換（`image` のみ送信）・sweep の flavor 既定適用・sweep の 400）/ set の image 再解決（`--image` 明示・`--image` の flavor 既定への優先・flavor 変更時の再解決・既定なし flavor への変更時の据え置き・flavor 未変更時の据え置き・同値時の変更なし報告・`SET` イベントへの記録・skip 時の非報告・一括時の単一値報告・一括で変更なし時の None）/ get_job の image 返却 / Prometheus カウンター（submit_job / submit_sweep で投入カウンター増加・cancel で完了カウンター増加・スキップ時は不変） |
| `tests/test_reconciler.py` | `watcher/reconciler.py` | 118 | reconcile_cycle のステータス同期（started_at / finished_at / last_error / node_name 記録・RUNNING スキップ時の完了時取得・既存値の非上書き）/ CANCELLED 削除 / orphan 検出 / DELETING フェーズ 1・2 / namespace 分離 / RUNNING 遷移時の累計消費量加算（namespace_daily_usage・flavor 別記録・異なる flavor の分離行）/ RUNNING スキップ時の完了フォールバック使用量記録（SUCCEEDED・FAILED・sweep parallelism 適用・RUNNING 観測済みジョブでの重複加算防止）/ K8s Job 消失検出（DISPATCHED・RUNNING → FAILED 遷移・last_error・finished_at 設定・dispatch grace period 内の DISPATCHED 保護・grace 超過後の FAILED 遷移・dispatched_at NULL 時の既存動作・RUNNING は grace 対象外）/ terminal 状態復帰ガード（FAILED / SUCCEEDED → RUNNING 遷移のブロック・DISPATCHED → RUNNING の通常遷移は許可） / 制限時間の強制（超過ジョブの FAILED 遷移・last_error・finished_at・K8s Job 削除・job_events 記録・failed カウンター増加・使用量の二重加算防止・未超過ジョブの非対象・naive timestamp の UTC 解釈・削除失敗時の RUNNING 維持・started_at NULL の非対象・terminal 状態の非対象・同一サイクル完了ジョブの保護・当該サイクル RUNNING 遷移ジョブの保護・k8s_job_name の k8s_map フォールバック・名前不明時のスキップ・sweep ジョブ・超過ジョブのみへの限定）/ DISPATCHED 滞留ガード（閾値超過ジョブの QUEUED 差し戻し・K8s Job 削除・UNSCHEDULABLE イベントと payload・requeue カウンター増加・初回バックオフ・2 回目の倍化・上限クランプ・巨大 attempt 時の指数クランプ・retry_count 非増加・使用量非加算・閾値内ジョブの非対象・dispatched_at NULL の非対象・K8s Job 不在時はステップ 8 に委譲・削除失敗時の DISPATCHED 維持・k8s_job_name の k8s_map フォールバック・当該サイクル RUNNING 遷移ジョブの保護・当該サイクル完了ジョブの保護・RUNNING ジョブの非対象・閾値 0 での無効化・naive timestamp の UTC 解釈・滞留ジョブのみへの限定）/ list_cjob_k8s_jobs の API エラー伝播・ページネーション（`_continue` トークン伝搬・ラベル不正ジョブのスキップ）・LightK8sJob 抽出 / NamespacePodNodeResolver の namespace 単位 Pod キャッシュ（単一 fetch 再利用・API エラー時の空結果キャッシュ・未知 job-name の空結果）/ parse_cpu_millicores / parse_memory_mib / Prometheus カウンター（SUCCEEDED / FAILED 遷移・K8s Job 消失で完了カウンター増加） |
| `tests/test_scheduler.py` | `dispatcher/scheduler.py` 6関数 | 37 | cas_update_to_dispatching / mark_dispatched / mark_failed / reset_stale_dispatching の CAS 動作・状態遷移 / mark_failed の Prometheus カウンター増加（成功時に increment・更新なし時は不変）/ filter_by_resource_quota の ResourceQuota 残リソースによる dispatch 候補フィルタリング（quota 行なし通過・CPU / メモリ / GPU 不足スキップ・sweep parallelism 倍計算・サイクル内累計追跡・namespace 混在・空リスト・count/jobs.batch 制限（NULL スキップ・充足通過・不足スキップ・累計追跡・sweep 1 カウント・リソース充足でも count 不足スキップ））/ fetch_dispatchable_jobs の fetch_limit パラメータ検証（DRF パス・フォールバックパス）・per-flavor DRF 容量の nominalQuota 考慮（nominalQuota による cap・allocatable 優先・quota なしフォールバック・複数 flavor の個別パラメータ）・DRF flavor weight の per-flavor パラメータ検証（weight は容量に乗じず別フィールドとして渡される） |
...
| `tests/test_node_bin_packing.py` | `dispatcher/scheduler.py::filter_by_node_capacity` | 27 | per-node bin-packing プレチェック。無効化フラグ / `node_resources` 空時の素通り / flavor 不在時の素通り / 単一ノード fit・no-fit / 複数ノード fit / GPU / memory チェック / 同サイクル累積追跡（least-loaded spread）/ RUNNING ジョブ控除（通常 node_name・listed ノードのみ・`node_name` NULL の least-loaded 代替・sweep 均等分配・余りあり）/ in-flight 控除（DISPATCHING / DISPATCHED の least-loaded 仮配置・sweep parallelism 分散）/ sweep admit（全 pod fit で admit・部分 fit で reject・1 ノードに集約）/ flavor 分離 / issue #203 シナリオ再現（300Gi × 2 ノード逼迫時の三つ目 reject・空き戻り時の通過）/ 複数 namespace 混在 |
...
| `tests/test_node_sync.py` | `watcher/node_sync.py::sync_node_resources` | 27 | ノードリソース同期。挿入 / 更新 / 削除 / 全削除 / GPU パース / API エラー時のデータ保持 / ラベルセレクタ / 部分失敗時の失敗 flavor データ保持 / 部分失敗時の成功 flavor 古ノード削除 / DaemonSet Pod request の差し引き（単一 Pod・複数 Pod 合算・複数コンテナ合算・非 DaemonSet Pod 除外・オーナー参照なし Pod 除外・Succeeded/Failed/Unknown phase 除外・Pending phase 計上・requests 未設定コンテナ 0 扱い・0 クランプ・複数ノード独立集計・Pod 取得 API エラー時のデータ保持・GPU 非適用・`_continue` トークンによるページ取得） |
...
| `tests/test_config.py` | `config.py::FlavorDefinition` / `Settings.flavors` | 10 | flavor 定義スキーマ。必須のみ / GPU 付き / `image` 付き / `image` のデフォルト None / `image` の null 指定 / 未知フィールドの拒否（`extra="forbid"`）/ 必須フィールド欠落 / `RESOURCE_FLAVORS` の JSON パースと `get_flavor_definition` / 未知フィールド時の `flavors` アクセス失敗 / デフォルト値の妥当性 |
