...
**データソース**:
- **Prometheus**: ノード/Pod メトリクス、Kueue メトリクス
- **PostgreSQL**: CJob DB（ジョブ状態、待ち時間実績、時間帯傾向）
...
### 2.3 Kueue Prometheus メトリクスの scrape 確認
...
```promql
# CPU 使用率ゲージ
kueue_cluster_queue_resource_usage{cluster_queue="cjob-cluster-queue", flavor="cpu-flavor", resource="cpu"}
/
kueue_cluster_queue_nominal_quota{cluster_queue="cjob-cluster-queue", flavor="cpu-flavor", resource="cpu"}
...
# メモリ使用量（GiB 変換）
kueue_cluster_queue_resource_usage{cluster_queue="cjob-cluster-queue", flavor="cpu-flavor", resource="memory"} / 1024 / 1024 / 1024

# メモリクォータ上限（GiB 変換）
kueue_cluster_queue_nominal_quota{cluster_queue="cjob-cluster-queue", flavor="cpu-flavor", resource="memory"} / 1024 / 1024 / 1024
```
