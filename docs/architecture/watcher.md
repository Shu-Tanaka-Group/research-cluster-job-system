...
2. Job の `status.conditions` を以下のルールで解釈する
   ...
   **Pending Pod の扱い:** K8s Job の `status.active` は "pending and running pods which are not terminating" の数であり、ノードリソース不足等で Pod が Pending のまま留まっている Job でも `active > 0` になる。Pending Pod を RUNNING と誤判定しないため、`status.ready`（Pod の `Ready` condition が True のもの、K8s 1.26 で GA となった `JobReadyPods` 機能）の併用で「実際にコンテナが Running 中の Pod」が存在することを確認する。cjob ジョブは readiness probe を持たないため、kubelet のデフォルト挙動により全コンテナが Running になった時点で Pod Ready は True となり、`ready > 0` は「少なくとも 1 つの Pod のコンテナが Running 中」と等価になる。`status.ready` は `list_job_for_all_namespaces()` のレスポンスに既に含まれているため、追加の K8s API / etcd 負荷は発生しない（最低 K8s バージョン要件は [prerequisites.md](prerequisites.md) §1 参照）。
...
最初の Pod が RUNNING になった時点（K8s Job の `status.active >= 1` かつ `status.ready >= 1`、§3 と同じ判定）で DB を RUNNING に更新する。通常ジョブと同様に `started_at` と `node_name` を記録する。
