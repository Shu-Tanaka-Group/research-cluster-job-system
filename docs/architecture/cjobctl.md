...
1. アクティブジョブ数を表示し、確認プロンプトを表示する（`--yes` でスキップ可）。`QUEUED` / `DISPATCHING` / `DISPATCHED` / `RUNNING` / `HELD` を個別にカウントして表示し、`HELD` が 1 件以上あれば「再起動後も HELD のまま残る」旨を併せて表示する
...
5. DB のジョブ状態を更新する:
   ...
   - QUEUED → 変更なし
   - HELD → 変更なし（再起動後もユーザーが `cjob release` するまで HELD のまま）
