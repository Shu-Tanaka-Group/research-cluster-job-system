...
| Dispatcher | DB スキャン → DRF ソート → 隙間充填フィルタ → ResourceQuota プレチェック → K8s Job 作成 | K8s API 呼び出し回数。シリアル実行のため 1 サイクルの処理時間に律速される。隙間充填と ResourceQuota プレチェックは DB 参照のみで K8s API を呼ばないため追加の外部 I/O は発生しない |
...
同時存在 Job 数は active Job + TTL 待ちの完了済み Job の合計である。`ttlSecondsAfterFinished = 300秒`（5 分）、`count/jobs.batch = 50` の場合：
...
|---|---|---|---|---|
| 長時間（平均 2h） | 32 | 1.3 | 33.3 | 3,330 |
| 中程度（平均 30m） | 32 | 5.3 | 37.3 | 3,730 |
| 短時間（平均 5m） | 32 | 32 | 64 → Quota 50 で制限 | 5,000 |

TTL を 300 秒に短縮したことで、長時間・中程度のジョブでは TTL 待ちの蓄積がほぼ無視でき、quota 50 に対して大幅な余裕がある。短時間ジョブ（平均 5 分）でも TTL 待ちは 32 件に収まるが、active 32 件との合計 64 件が quota 50 を超えるため安全弁として機能する。quota に達した場合は TTL 経過で自然に回復し、Dispatcher の retry により自動復旧する。
...
`DISPATCH_BUDGET_PER_NAMESPACE = 32`、`ttlSecondsAfterFinished = 300秒`、`count/jobs.batch = 50` の場合の推定。

| ワークロード | 推定上限ユーザー数 | 律速要因 |
|---|---|---|
| 長時間ジョブ中心（30 分〜数時間） | **150〜200 名** | K8s 同時存在 Job 数（150 名で約 5,000〜5,600） |
| 混在（長時間 + sweep） | **120〜150 名** | K8s 同時存在 Job 数 + Kueue admission |
| 短時間ジョブ中心（数分） | **80〜100 名** | Watcher 検知遅延 + Dispatcher スループット |

TTL 300 秒により TTL 待ちの蓄積が大幅に減少し、長時間ジョブ中心（合計/ユーザー ≈ 33）では 150 ユーザーでも同時存在 Job 数 5,000 程度に収まる。短時間ジョブ中心の場合でも `count/jobs.batch = 50` が安全弁として機能し、同時存在 Job 数を 5,000 以下に抑える。ただし Watcher の list 負荷と検知遅延が律速となるため、Watch API 移行との併用が望ましい。
...
budget 16 であれば 200 ユーザーでも active Job 数 3,200 であり、TTL 300 秒での TTL 待ちを含めても K8s の実用上限に対して十分な余裕がある。budget 8 であれば 200 ユーザーでも active Job 数 1,600 と余裕で、同時存在 Job 数は問題にならない。
...
|---|---|---|
| 現状（ポーリング + budget 32） | - | 150〜200 名 |
| Watch API 移行のみ | Watcher の read 負荷軽減、検知遅延解消 | 200〜250 名 |
| budget 16 のみ | active Job 数半減 | 250〜300 名 |
| Watch API + budget 16 | 両方の効果 | 300〜400 名 |
