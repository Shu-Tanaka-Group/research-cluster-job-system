...
| 20 | 32 | 640 | 余裕 |
| 50 | 32 | 1,600 | 余裕 |
| 100 | 32 | 3,200 | 注意（TTL 待ち込みで 5,000 超の可能性） |
| 150 | 32 | 4,800 | 上限付近 |
| 200 | 32 | 6,400 | 超過リスク |
...
`DISPATCH_BUDGET_PER_NAMESPACE = 32` の場合、100 ユーザーで active Job 数 3,200。TTL 待ちの完了済み Job を含めると 5,000 を超える可能性があり、K8s 標準構成の実用上限（5,000〜10,000）に到達し始める。
...
`DISPATCH_BUDGET_PER_NAMESPACE = 32` の場合の推定。

| ワークロード | 推定上限ユーザー数 | 律速要因 |
|---|---|---|
| 長時間ジョブ中心（30 分〜数時間） | **100〜150 名** | K8s 同時存在 Job 数 |
| 混在（長時間 + sweep） | **80〜120 名** | K8s 同時存在 Job 数 + Kueue admission |
| 短時間ジョブ中心（数分） | **50〜80 名** | Watcher 検知遅延 + Dispatcher スループット |

長時間ジョブ中心の場合、同時存在 Job 数が K8s の実用上限に達するまで余裕があるため、最も多くのユーザーを収容できる。短時間ジョブ中心の場合、ジョブの回転が速いため Watcher と Dispatcher の処理速度が律速となり、上限が下がる。
...
budget 16 であれば 200 ユーザーでも active Job 数 3,200 であり、TTL 待ち込みでも K8s の実用上限に対して十分な余裕がある。budget 8 であれば 200 ユーザーでも active Job 数 1,600 と余裕で、同時存在 Job 数は問題にならない。
