...
**Watcher の list 負荷**: 10 秒ごとの `list_job_for_all_namespaces()` で取得する Job 数は、100 ユーザーで最大 3,300〜5,000 件（1 Job ≈ 3KB として 10〜15MB/回）。大きいが動作不能になる水準ではない。Watch API 移行により解消可能（§6.5 参照）。
