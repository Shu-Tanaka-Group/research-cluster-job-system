...
|---|---|---|
| 長時間ジョブ中心（30 分〜数時間） | **100〜150 名** | K8s 同時存在 Job 数（100 名で約 3,300〜3,700） |
| 混在（長時間 + sweep） | **100〜120 名** | K8s 同時存在 Job 数 + Kueue admission |
| 短時間ジョブ中心（数分） | **80〜100 名** | Watcher 検知遅延 + Dispatcher スループット |

TTL 300 秒により TTL 待ちの蓄積が大幅に減少し、長時間ジョブ中心では 100 ユーザーで同時存在 Job 数 3,300 程度に収まる。短時間ジョブ中心の場合でも `count/jobs.batch = 50` が安全弁として機能し、同時存在 Job 数を 5,000 以下に抑える。ただし Watcher の list 負荷と検知遅延が律速となるため、Watch API 移行との併用が望ましい。
