...
CLI バイナリを最新バージョンに更新する。バイナリは Submit API 経由で配布される。

1. `GET /v1/cli/version` で最新バージョンを取得する
2. ローカルの CLI バージョン（`--version` で表示されるもの）と比較する
3. 同一バージョンであれば「すでに最新です」と表示して終了する
4. 新しいバージョンがある場合:
   1. `GET /v1/cli/download` でバイナリをダウンロードする
   2. 現在の実行ファイルを新しいバイナリで置き換える
   3. 置き換え後にファイルに実行権限（`0o755`）を付与する
   4. 更新完了メッセージを表示する
