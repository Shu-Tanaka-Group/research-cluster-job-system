...
- **weight = 0**: dispatch 対象から除外される（使用禁止）。ジョブは QUEUED に留まり、weight を 0 より大きい値に戻すと dispatch が再開される。管理者が特定ユーザーにクラスタ全体を専有させたい場合に、他ユーザーの weight を 0 に設定することで実現できる
- **weight > 0**: weight が大きい namespace ほど、同じ累計消費量でも dominant share が小さく評価され、dispatch 優先度が高くなる。例えば weight = 2 の namespace は、weight = 1 の namespace より多くのリソースを使い切るまで優先的に dispatch される。実数値（例: 1.5）を指定可能
...
| `flavor` | TEXT | ResourceFlavor 名。DRF 計算時に `flavor_quotas.drf_weight` を乗じるために記録する |
...
-- (namespace, flavor) 単位で集計する（drf_weight は Dispatcher 側で GREATEST 計算後に適用する）
...
Dispatcher が `fetch_dispatchable_jobs()` を実行する直前に、保持期間外の古い行を削除する。保持期間は `USAGE_RETENTION_DAYS`（デフォルト 7）で制御する。DRF 計算ウィンドウ（`FAIR_SHARE_WINDOW_DAYS`）とは独立しており、将来 DRF 以外の用途で消費量データを参照する場合に、より長い期間のデータを保持できる。
...
- **flavor 列**: DRF 計算で flavor ごとに独立して dominant share を算出するため、flavor 単位で消費量を分離して記録する。`drf_weight` は記録時ではなく計算時に適用することで、weight 変更が既存の履歴データに即座に反映される
- **保持期間の分離**: `USAGE_RETENTION_DAYS` を `FAIR_SHARE_WINDOW_DAYS` と独立させることで、DRF 以外の用途（利用統計等）で消費量データを参照する際に、より長い期間のデータを保持できる
- **行数の見積もり**: namespace 数 × ウィンドウ日数 × flavor 数。20 namespace × 7 日 × 3 flavor = 420 行程度であり、集計クエリのコストは無視できる
...
Python 側で flavor ごとに `MIN(allocatable, nominalQuota)` を計算し、`drf_weight` と合わせて per-flavor 容量として保持する（`_fetch_flavor_caps()` 関数）。nominalQuota の TEXT 値は `parse_cpu_millicores()` / `parse_memory_mib()` でパースする。
...
`drf_weight` は DRF 計算時にこの flavor のリソース消費量・容量に乗じる重み係数である。デフォルト 1.0。GPU など貴重なリソースには大きい値（例: 2.0）、低スペック flavor には小さい値（例: 0.5）を設定する。Watcher の sync は `cpu`・`memory`・`gpu`・`updated_at` のみを更新するため、`drf_weight` は干渉されない。`cjobctl cluster set-drf-weight` で設定する。
...
**Dispatcher（DRF flavor weight）**: flavor ごとの容量（`MIN(allocatable, nominalQuota)`）と `drf_weight` を取得し、flavor 単位の dominant share 計算に使用する。`drf_weight` は消費量や容量に直接乗じるのではなく、flavor ごとの dominant share（`GREATEST(cpu_share, mem_share, gpu_share)`）に乗じてから全 flavor で合算する。

```sql
SELECT flavor, cpu, memory, gpu, drf_weight FROM flavor_quotas;
```

**cjobctl（DRF weight 設定）**: 指定 flavor の `drf_weight` を更新する。
...
CREATE TABLE namespace_resource_quotas (
    ...
    hard_count           INTEGER,              -- count/jobs.batch の hard 値。NULL = 制限なし
    used_cpu_millicores  INTEGER NOT NULL,
    used_memory_mib      INTEGER NOT NULL,
    used_gpu             INTEGER NOT NULL DEFAULT 0,
    used_count           INTEGER,              -- count/jobs.batch の used 値。NULL = 制限なし
    updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
```
...
`hard_count` / `used_count` は `count/jobs.batch` ResourceQuota の値を格納する。ResourceQuota に `count/jobs.batch` が設定されていない場合は両カラムとも `NULL` とし、Dispatcher は Job 数制限なしとして扱う。CPU/memory/GPU が未設定時に `0` を格納するのとは異なり、`count/jobs.batch` は ResourceQuota で任意設定であるため `NULL` で「未設定」を明示する。
...
`count/jobs.batch` は `spec.hard` から直接取得する。`spec.hard` に `count/jobs.batch` が含まれている場合は `hard_count` に整数値を、`status.used` から `used_count` に整数値を格納する。`spec.hard` に含まれていない場合は両カラムとも `NULL` を格納する。
