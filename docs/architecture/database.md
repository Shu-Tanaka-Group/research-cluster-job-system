...
-- flavor_quotas.drf_weight を乗じることで flavor ごとの価値の違いを反映する
SELECT u.namespace,
...
- **flavor 列**: DRF 計算時に `flavor_quotas.drf_weight` を乗じるため、flavor ごとに消費量を分離して記録する。weight を記録時ではなく計算時に適用することで、weight 変更が既存の履歴データに即座に反映される
...
**Dispatcher（DRF flavor weight）**: DRF 正規化の分母と消費量の両方に `drf_weight` を乗じる。nominalQuota と合わせて取得する。
