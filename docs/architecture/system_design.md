...
### 2.7 ResourceFlavor による計算リソースの分類
...
- **Kueue の ResourceFlavor 機構を活用する**: 各 flavor は Kueue の `ResourceFlavor` オブジェクトに対応し、`nodeLabels` によって対象ノードが決定される。Dispatcher は `nodeSelector` を設定せず、ノードの振り分けは Kueue に委ねる
