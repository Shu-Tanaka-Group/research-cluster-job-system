...
- **Max Node Allocatable**: flavor ごとの最大ノード allocatable 値。Submit API のリソース超過リジェクトでは nominalQuota と合わせて `min(最大ノード allocatable, nominalQuota)` が有効上限となる
...
## 7. 既存 flavor への計算ノードの追加

### 7.1 ノードのラベル・taint の付与

新しいノードには以下のラベルと taint を付与する。taint の値は ConfigMap `cjob-config` の `JOB_NODE_TAINT` に合わせること（デフォルト: `role=computing:NoSchedule`）。

```bash
kubectl label node <node-name> cjob.io/flavor=<flavor名>
kubectl taint node <node-name> role=computing:NoSchedule
```

ラベルは ConfigMap `cjob-config` の `RESOURCE_FLAVORS` で定義された該当 flavor の `label_selector` と一致している必要がある。全 flavor で共通キー `cjob.io/flavor` を使用し、値に flavor 名を設定する。
...
既存の flavor に属さないノード（異なる CPU アーキテクチャや異なる GPU モデルなど）を追加する場合、新しい flavor を作成する必要がある。

### 8.1 ノードにラベルと taint を付与する
...
taint を使わない運用（他のワークロードとノードを共有する場合）では taint の付与を行わない。

### 8.2 Kueue ResourceFlavor を作成する
...
spec:
  nodeLabels:
    cjob.io/flavor: "<flavor名>"    # §8.1 で付与したラベルと一致させる
  nodeTaints:               # taint を使わない場合は省略
    - key: "role"
      value: "computing"
      effect: "NoSchedule"
  tolerations:              # taint を使わない場合は省略
    - key: "role"
      operator: "Equal"
      value: "computing"
      effect: "NoSchedule"
...
| `DISPATCH_FETCH_MULTIPLIER` | SQL 候補取得数の倍率（`DISPATCH_BATCH_SIZE × 倍率` を取得し、フィルタ後に `DISPATCH_BATCH_SIZE` 件へ絞り込む） |
| `DISPATCH_ROUND_SIZE` | ラウンドロビンと DRF（消費量ベースの公平性）のバランス制御 |
| `FAIR_SHARE_WINDOW_DAYS` | DRF の消費量集計ウィンドウ日数 |
| `USAGE_RETENTION_DAYS` | `namespace_daily_usage` の保持日数（`FAIR_SHARE_WINDOW_DAYS` と独立） |
...
| `ctl/src/cmd/counters.rs` | `cjobctl jobs counters` |
