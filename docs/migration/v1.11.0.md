...
## Dispatcher Prometheus メトリクスの有効化

Dispatcher に Prometheus メトリクスエンドポイントを追加した（issue #128）。`kubectl apply -k` で Deployment の Prometheus annotations・metrics port と PodMonitor（`podmonitor-dispatcher.yaml`）が適用される。

Prometheus Operator を使用していない環境では、PodMonitor の代わりに scrape 設定を手動で追加する必要がある。Deployment の annotations（`prometheus.io/scrape: "true"`, `prometheus.io/port: "9090"`）を利用できる。
