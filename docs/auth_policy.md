...
各 namespace に `cjob-user` という名前の ServiceAccount を作成する。
...
> **注**: 以下の YAML は認証設計の参照用として記載する。実際に適用する YAML の正本は `deployment.md` §8 である。
...
> **注**: 以下の YAML は認証設計の参照用として記載する。実際に適用する YAML の正本は `deployment.md` §10 である。
...
[ユーザーログイン]
  Keycloak で認証 → JupyterHub が User Pod を作成
    └─ spec.serviceAccountName: cjob-user
