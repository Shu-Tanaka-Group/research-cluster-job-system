          ...
          env:
            ...
            - name: TTL_SECONDS_AFTER_FINISHED
              valueFrom:
                configMapKeyRef:
                  name: cjob-config
                  key: TTL_SECONDS_AFTER_FINISHED
            - name: JOB_NODE_TAINT
              valueFrom:
                configMapKeyRef:
                  name: cjob-config
...
### 15.1 ClusterQueue リソースメトリクスの有効化
...
- Kueue の ClusterQueue リソースメトリクスが有効化されていること（§15.1 参照）
- Kueue のメトリクスが Prometheus で scrape されていること
