...
- `LOG_BASE_DIR` のデフォルト値（`/home/jovyan/.cjob/logs`）は `WORKSPACE_MOUNT_PATH`（デフォルト `/home/jovyan`）配下のパスである。`WORKSPACE_MOUNT_PATH` を変更する場合は `LOG_BASE_DIR` も合わせて変更すること。`LOG_BASE_DIR` が `WORKSPACE_MOUNT_PATH` の外を指すとログが PVC に書き込まれず消失する
...
- ClusterRole `cjob-job-controller` は Job の CRUD、Pod の読み取り、Node の読み取り、Namespace の一覧取得、ResourceQuota の一覧取得、ClusterQueue の読み取りを許可する
...
- ClusterQueue の `get` 権限（`kueue.x-k8s.io` API グループ）は Watcher が `flavor_quotas` テーブルに nominalQuota を同期するために必要
