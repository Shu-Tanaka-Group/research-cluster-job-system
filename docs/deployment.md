...
- ClusterRole `cjob-job-controller` は Job の CRUD、Pod の読み取り、Node の読み取り、ResourceQuota の読み取りを許可する
- Node の `get`/`list` 権限は Watcher が `node_resources` テーブルにノード情報を同期するために必要
- ResourceQuota の `get` 権限は Watcher が `namespace_resource_quotas` テーブルに ResourceQuota 使用状況を同期するために必要

---

## 10. NetworkPolicy

ソース: [`networkpolicy-allow-submit-api.yaml`](../k8s/base/networkpolicy-allow-submit-api.yaml)

設計上の要点:
- このクラスタには default-deny NetworkPolicy は存在しないため、`cjob-system` namespace 内の Pod 間通信（Submit API ↔ PostgreSQL など）は制限しない
- User namespace（`cjob.io/user-namespace: "true"` ラベル付き）から Submit API（TCP 8080）への通信のみを許可する
- User namespace 以外からの Submit API へのアクセスを制限することでセキュリティを確保する
