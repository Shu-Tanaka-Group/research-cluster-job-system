...
ジョブが長時間 `QUEUED`（待機中）や `DISPATCHED`（リソース割当待ち）のままになることがあります。多くはクラスタが混んでいるだけですが、`cjob status` の履歴を見ると理由を絞り込めます。
...
`UNSCHEDULABLE` は「クラスタ側に空きノードが見つからず、ジョブが実行開始できないまま待機し続けたため、システムが待機列に戻した」ことを表します。ジョブは失われておらず、`retry_after` に表示された時刻を過ぎると自動的に再び実行が試みられます。

- 何もしなくても、クラスタに空きができた時点で自動的に実行が始まります
- `UNSCHEDULABLE` が何度も記録される場合は、要求しているリソース（`--cpu` / `--memory` / `--gpu`）が 1 ノードに収まりにくい大きさになっている可能性があります。要求を小さくすると実行開始が早まることがあります
- 待つのをやめる場合は `cjob cancel <job_id>` でキャンセルできます

`retry_after` が表示されていても、履歴が `RETRY` や `DEFERRED` の場合は数十秒程度の一時的な待機であり、そのまま待てば実行されます。
