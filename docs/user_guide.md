...
| 同時に実行できるジョブ数 | flavor ごとに 32 件 | 超過分は待機状態で順番を待つ |
...
上限に達した場合は、`cjob delete` で完了済み・キャンセル済みのジョブを削除してください。実行中のジョブについては、完了を待つか `cjob cancel` でキャンセルした上で削除してください。ジョブの削除について詳しくは[完了済みジョブの削除](../README.md#完了済みジョブの削除)を参照してください。

### 1.2 同時に実行できるジョブ数の上限（flavor ごとに 32 件）

一度に実行状態になれるジョブは、**flavor ごとに最大 32 件** です。たとえば CPU ジョブと GPU ジョブはそれぞれ独立して 32 件まで同時に実行できます。CPU ジョブが 32 件実行中でも、GPU ジョブの枠には影響しません。

33 件目以降のジョブは待機状態（QUEUED）のまま順番を待ち、同じ flavor の実行中のジョブが完了すると自動的に次のジョブが開始されます。
