...
- **新コードが先に必要とするものを前倒しする**（例: 「DB スキーマ更新を K8s リソース適用より先に」）— 新コードが未追加のカラムや設定キーを参照する場合に発生する。標準手順の Step 順を**前倒し**する方向の逸脱
- **旧コードが読めないものを後ろ倒しする**（例: 「`RESOURCE_FLAVORS` への新フィールド追加は新コード適用の後」）— 設定値のスキーマを拡張したとき、拡張後の値を旧コードが読むと起動失敗・誤動作する場合に発生する。標準手順の Step 順を**後ろ倒し**する方向の逸脱。ConfigMap と Deployment は同じ `kubectl apply -k`（Step 4）で同時に適用されるため、**値の更新だけを Step 4 から分離**する必要がある点に注意する
- **コンポーネント間の順序**（例: 「Watcher を Dispatcher より先に」）— データを生産する側を先にデプロイする
- **手動作業の位置**（例: 「Grafana の再インポートは DB スキーマ更新の後」）
- **ロールアウト前の事前確認**（例: 「バリデーション厳格化の前に現行の設定値が新しい規則を満たすか確認する」）— 満たさない場合に全コンポーネントが起動失敗し、設定を直すまでロールバックも効かない類のもの。**Step 1 より前**に置く

上 2 つは向きが逆であり、同一リリースに同居しうる。前倒しと後ろ倒しの両方が存在する場合は、それぞれ独立した項目として runbook に並べる（片方に引きずられて他方を同じ向きに揃えない）。
