...
4. **CLI インターフェース** — サブコマンド・引数名・short/long flag・出力フォーマット（JSON フィールド、human-readable の列順）・バリデーションルールが設計書と Rust 実装で一致しているか
   - **出力メッセージ文字列の実在確認**: 設計書の出力例ブロック（`$ cjob ...` / `$ cjobctl ...` を含むコードブロック）と、本文が `「...」` や `"..."` で引用しているメッセージについて、その文字列が実装側の文字列リテラルとして実在するかを確認する
     - CLI 自身が出力するメッセージは `cli/src/` / `ctl/src/` のリテラルと照合する（`println!` / `eprintln!` / `bail!` / `anyhow!` / `.context(...)` の引数）
     - Submit API 由来のメッセージ（CLI がサーバのエラーをそのまま表示するもの）は `server/src/cjob/api/` のリテラルと照合する。**両者は言語が異なる**（CLI は英語、API は日本語）ため、どちら由来かを見極めてから照合先を選ぶこと
     - 照合はフォーマット指定子（`{}` / `{:?}` / `{:<16}` 等）を除いた固定部分で行う。設計書の出力例では指定子が実際の値（job_id、バージョン番号、日時、パス等）に展開されているため、完全一致では照合できない
     - 例示値そのもの（`1.3.0`、`worker07`、`2026-03-23 12:35:00` 等）の食い違いは finding にしない。固定のメッセージ文言・ラベル・列見出しのみを対象とする
     - 失敗モード: メッセージ文言だけを変更し設計書の出力例を更新し忘れる。実例として #166（CLI メッセージの一括英語化）が `cli/` のみを変更し、`docs/` の出力例が日本語のまま 5 か月間取り残された（#215 で修正）
