...
| `src/main.rs` | `parse_duration` / `parse_time_limit_range` / `shell_quote`, etc. | 35 | Parsing time specifications (seconds / s / m / h / d suffixes / whitespace / invalid values / overflow) / parsing time_limit range specifications (h / m / d / mixed units / seconds / open-ended / no colon / empty range / invalid values / lower-bound >= upper-bound error) |
...
| `src/cmd/config/set.rs` | `validate_resource_flavors` / `validate_against_configmap` | 26 | Structural validation of `RESOURCE_FLAVORS`. Success cases (names returned in definition order / `gpu_resource_name` omitted or null / multi-line ConfigMap value) / invalid JSON / non-array / empty array / non-object element / required field missing, empty, or non-string / empty `gpu_resource_name` / unknown field / duplicate `name` / `label_selector` form (no `=`, two `=`, one side empty) / all violations reported together. Consistency with `DEFAULT_FLAVOR` (when setting `RESOURCE_FLAVORS`: match, mismatch, warning when unset, surrounding whitespace ignored; when setting `DEFAULT_FLAVOR`: match, mismatch, warning and skip when `RESOURCE_FLAVORS` is unset or broken) / unrelated keys pass through |

**Total: Python 491 + Python integration 33 + Rust (cli) 68 + Rust (cjobctl) 54 = 646 tests**
