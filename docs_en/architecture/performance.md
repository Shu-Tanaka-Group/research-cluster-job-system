...
| Jobs stall in DISPATCHED due to insufficient ResourceQuota | ResourceQuota pre-check handles this automatically (already implemented, see [dispatcher.md](dispatcher.md) §2.5). Adjust the sync interval via `RESOURCE_QUOTA_SYNC_INTERVAL_SEC` |
| Jobs stall in DISPATCHED because they fit total quota but cannot be placed on any single node | per-node bin-packing pre-check handles this automatically (already implemented, see [dispatcher.md](dispatcher.md) §2.6). Toggle on/off via `NODE_BIN_PACKING_ENABLED` |
| A job that slipped past the pre-checks stalls in DISPATCHED for a long time and holds quota | The Watcher's DISPATCHED stall guard handles this automatically (already implemented, see [watcher.md](watcher.md) §3 Step 10). Tune the stall tolerance via `WATCHER_DISPATCH_TIMEOUT_SEC` and the backoff ceiling via `WATCHER_DISPATCH_BACKOFF_MAX_SEC` |
