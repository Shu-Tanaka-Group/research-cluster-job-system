...
📦 **簡単なジョブ投入** — `cjob add -- python main.py` だけでジョブを投入
...
- 詳細はユーザーガイドの[sweep を使った効率的なジョブ実行](./docs/user_guide.md#2-sweep-を使った効率的なジョブ実行)をご参照ください
...
- リソース指定についての詳細はユーザーガイドの[CPUコア数に関する説明](./docs/user_guide.md#6-プログラムが使う-cpu-コア数と---cpu-の合わせ方)をご参照ください
...
- `--flavor` はジョブを実行する計算リソースの種類を指定するオプションです。CPU ノードや GPU ノードなど、異なる種類のノードを使い分けることができます
- `--flavor` を省略するとデフォルトの種類が選択されます
- 利用可能な種類は `cjob flavor list` で確認できます
- 詳細はユーザーガイドの[計算リソースの種類を指定する](./docs/user_guide.md#3-計算リソースの種類を指定する--flavor)をご参照ください
...
- 詳細はユーザーガイドの[リソース使用状況の確認](./docs/user_guide.md#7-リソース使用状況の確認cjob-usage)をご参照ください
...
```bash
# 待機中・保留中のジョブの flavor やリソースを変更する
$ cjob set <job-ids> --flavor cpu-sub --cpu 4 --memory 16Gi --time-limit 12h
```

- 指定したオプションのみ更新し、未指定の項目は現在値を維持します
- 詳細はユーザーガイドの[投入済みジョブのパラメータを変更する](./docs/user_guide.md#5-投入済みジョブのパラメータを変更するcjob-set)をご参照ください
...
> [!NOTE] > `cjob delete` は完了済みジョブを即座に削除します。
